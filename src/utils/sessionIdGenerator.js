...
function extractSessionId(chatId, senderId) {
  // 使用用户 ID + 群组 ID 作为会话标识（不使用时间戳，确保同一会话的 ID 一致）
  const userId = typeof senderId === 'object' ? (senderId.user_id || senderId.open_id || 'unknown') : (senderId || 'unknown');
  const chatIdStr = chatId || 'unknown';
  const hash = crypto.createHash('md5').update(`${userId}_${chatIdStr}`).digest('hex').substring(0, 12);
